...
async function initializeDragUpload() {
  if (
    game.user.isGM ||
    game.user.hasPermission(CONST.USER_PERMISSIONS.FILES_UPLOAD)
  ) {
...
async function createFoldersIfMissing() {
  ...
  const targetLocationFolders = targetLocation
    .split("/")
    .filter((x) => x !== "");
  let pathParts = [];
  ...
  try {
    ...
    if (!result.dir.includes(folderPath))
      await FilePicker.createDirectory(
        ...
        folderPath,
        source === "s3"
          ? { bucket: game.settings.get("dragupload", "fileUploadBucket") }
          : {}
      );
  } catch (error) {
    try {
...
async function handleDrop(event) {
  ...
  if (!files || files.length === 0) {
    ...
    if (url.includes("imgur") && filename.endsWith("_d.webp")) {
      filename =
        filename.substr(0, filename.length - "_d.webp".length) + ".png";
  ...
  if (
    Object.keys(CONST.AUDIO_FILE_EXTENSIONS).filter(
      (x) => x != "webm" && file.name.endsWith(x)
    ).length > 0
  ) {
    ...
    if (files.length > 1) {
      ...
      if (types.length > 1 && !actorType) {
        ...
        types.forEach((x) => {
          d.data.buttons[x] = {
            label: x,
            callback: async () => batchUpload(event, files, x),
          };
        });

        d.render(true);
      } else {
        batchUpload(event, files, types[0]);
      }
...
async function batchUpload(event, files, type) {
  return Promise.all(
    Object.values(files).map(async (f) => {
      return CreateActor(event, f, type)
    })
  );
}
...
async function CreateTile(event, file, overhead) {
  ...
  const ratio =
    canvas.dimensions.size / (data.tileSize || canvas.dimensions.size);
  data.texture = {
    src: data.img,
  };
  ...
  if (!event.shiftKey)
    foundry.utils.mergeObject(
      data,
      canvas.grid.getSnappedPosition(data.x, data.y, 1)
    );
  ...
  if (game.release?.generation <= 9) {
    ...
    canvas.perception.update(
      { refreshLighting: true, refreshTiles: true },
      true
    );
  }
...
async function CreateActor(event, file, actorType) {
  ...
  if (
    Object.keys(CONST.IMAGE_FILE_EXTENSIONS).filter((x) =>
  ...
  if (types.length > 1 && !actorType) {
    ...
    types.forEach((x) => {
      d.data.buttons[x] = {
        label: x,
        callback: async () =>
          await CreateActorWithType(event, data, tokenData, x),
      };
    });

    d.render(true);
  } else if (actorType) {
    await CreateActorWithType(event, data, tokenData, actorType);
  } else {
...
async function CreateActorWithType(event, data, tokenImageData, type) {
  ...
  const hasPack = game.packs.get("world.upload") !== undefined;
  const actor = await getDocumentClass("Actor").create({
    name: actorName,
    type: createdType,
    img: data.img,
  }, {pack: hasPack ? 'world.upload' : undefined});
  ...
  if (
    ...
    !canvas.grid.hitArea.contains(tokenData.x, tokenData.y)
  )
    return false;
  if (game.release?.generation > 9) {
  ...
  tokenData = foundry.utils.mergeObject(
    foundry.utils.duplicate(prototypeToken),
    tokenData,
    { inplace: true }
  );
